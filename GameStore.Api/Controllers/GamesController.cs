...
public class GamesController : ControllerBase
{
    ...
    )
    {
        ...
        {
            query = query.Where(v => v.Genres.Any(g => genreIds.Contains(g.Id)));
        }

        // 🔎 FILTRO por plataformas
        if (platformIds is { Length: > 0 })
        {
            query = query.Where(v => v.Platforms.Any(p => platformIds.Contains(p.Id)));
        }
        ...
        return Ok(
            new PaginatedResponse<VideogameDTO>
            {
                Page = page,
                PageSize = pageSize,
                Total = total,
    ...
    {
        var platforms = await _db.Platforms.AsNoTracking().OrderBy(p => p.Name).ToListAsync();
        return Ok(platforms);
    }
    ...
    public async Task<IActionResult> Create([FromBody] CreateVideogameRequestDto createDto)
    {
        ...
        var genres = await _db.Genres.Where(g => createDto.GenreIds.Contains(g.Id)).ToListAsync();
        ...
        var platforms = await _db
            .Platforms.Where(p => createDto.PlatformIds.Contains(p.Id))
            .ToListAsync();
    ...
    public async Task<IActionResult> Update(int id, [FromBody] UpdateVideogameRequestDto updateDto)
    {
        ...
        {
            var newGenres = await _db
                .Genres.Where(g => updateDto.GenreIds.Contains(g.Id))
                .ToListAsync();
        ...
        {
            var newPlatforms = await _db
                .Platforms.Where(p => updateDto.PlatformIds.Contains(p.Id))
                .ToListAsync();
